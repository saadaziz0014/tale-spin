...
import logo from "../assets/Tale-Spin.svg";
...
export default function Signin() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.log(dataRes);
      let user = {
        _id: dataRes.payload.id,
        email: dataRes.payload.email,
        name: dataRes.payload.name,
        token: dataRes.token,
      };
      console.log(user);
    ...
    <section className="rounded-md bg-black/70 p-2">
      <div className="flex items-center justify-center bg-white px-4 py-10 sm:px-6 sm:py-16 lg:px-8">
        <div className="xl:mx-auto xl:w-full xl:max-w-sm 2xl:max-w-md">
          <div className="mb-2">
            <Link to="/">
              <img src={logo} alt="Tale Spin Logo" className="w-20 h-20" />
            </Link>
          </div>
